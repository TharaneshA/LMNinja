...
import { Plus, Trash2, CheckCircle, Cloud, Server, Loader2, Wand2 } from "lucide-react";
import { toast } from "sonner";
import { SaveConnection, DeleteConnection, GetProviderModels } from "@wailsjs/go/app/App"; // Use alias
import { storage } from "@wailsjs/go/models"; // Use alias

type FormData = { id: string; name: string; provider: string | ""; model: string; apiKey: string; }
const emptyForm: FormData = { id: "", name: "", provider: "", model: "", apiKey: "" };
...
export function Connectors() {
  ...
  const handleConnectionSelect = (connection: storage.ConnectionMetadata) => {
    ...
    setAvailableModels([connection.model]);
  };
  ...
  const handleFetchModels = async () => {
    ...
    try {
      ...
      if (!models || models.length === 0) {
        ...
        toast.success(`Found ${models.length} models for ${formData.provider}.`);
      }
    } catch (error) {
      toast.error("Failed to fetch models", { description: String(error) });
    } finally {
  ...
  const handleSaveConnection = async () => {
    if (!formData.name || !formData.provider || !formData.model) {
      toast.warning("Please fill in a name, select a provider, and select a model."); return;
    }
    if (!formData.id && !formData.apiKey) {
      toast.warning("API Key is required for new connections."); return;
    }
    setIsSaving(true);
    try {
      const meta = new storage.ConnectionMetadata({
        id: formData.id,
        name: formData.name,
      ...
      toast.error("Failed to save connection", { description: String(error) });
    } finally {
      setIsSaving(false);
    }
  };
  ...
  const handleDeleteConnection = async (id: string, name: string) => {
    ...
    try {
      ...
      toast.error("Failed to delete connection", { description: String(error) });
    }
  };
    ...
    <div className="p-6">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <Card className="bg-[#1B2636] border-[#2A3B52] lg:col-span-1">
          <CardHeader><CardTitle className="flex items-center justify-between"><span>My Models</span><Button size="sm" className="bg-[#4A90E2] hover:bg-[#3A7BC8]" onClick={handleNewConnection}><Plus className="w-4 h-4 mr-1" /> New</Button></CardTitle></CardHeader>
          <CardContent>
            {connections.length === 0 ? (<div className="text-center text-gray-400 py-8 px-4"><p className="font-semibold">No models connected.</p><p className="text-sm mt-1">Click 'New' to add your first connection.</p></div>) : (
              <div className="space-y-3">{connections.map((connection) => (<div key={connection.id} className={`p-3 rounded-lg cursor-pointer transition-colors ${selectedConnectionId === connection.id ? "bg-[#4A90E2] bg-opacity-20 border border-[#4A90E2]" : "bg-[#0F1419] hover:bg-[#2A3B52]"}`} onClick={() => handleConnectionSelect(connection)}><div className="flex items-center justify-between"><div className="flex items-center space-x-3"><CheckCircle className="w-4 h-4 text-green-500" /><div><p className="text-sm font-medium">{connection.name}</p><p className="text-xs text-gray-400 uppercase">{connection.provider}: {connection.model}</p></div></div><Button size="icon" variant="ghost" className="h-7 w-7 hover:bg-red-500/20 text-gray-400 hover:text-red-400" onClick={(e) => { e.stopPropagation(); handleDeleteConnection(connection.id, connection.name)}}><Trash2 className="w-3.5 h-3.5" /></Button></div></div>))}</div>)}
          </CardContent>
        </Card>
        <Card className="bg-[#1B2636] border-[#2A3B52] lg:col-span-2">
          ...
          <CardContent>
            <Tabs defaultValue="cloud" className="w-full">
              <TabsList className="grid w-full grid-cols-2 mb-6"><TabsTrigger value="cloud" className="flex items-center space-x-2"><Cloud className="w-4 h-4" /> Cloud API</TabsTrigger><TabsTrigger value="local" className="flex items-center space-x-2" disabled><Server className="w-4 h-4" /> Local Model (Soon)</TabsTrigger></TabsList>
