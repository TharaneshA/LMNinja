...
import { app, storage } from "@wailsjs/go/models";
...
export function Connectors() {
  ...
  const handleConnectionSelect = (connection: storage.ConnectionMetadata) => {
    ...
    setFormData({ id: connection.id, name: connection.name, provider: connection.provider, model: connection.model, apiKey: "" });
  ...
  const handleFetchModels = async () => {
    if (!formData.provider || !formData.apiKey) { toast.warning("Please select a provider and enter an API key first."); return; }
  ...
  const handleSelectGGUFFolder = async () => {
    ...
    try {
      ...
      if (files && files.length === 0) { toast.info("No GGUF files found in the selected folder.");
      } else if (files && files.length > 0) { toast.success(`Discovered ${files.length} GGUF model(s).`); }
    } catch (error) {
  ...
  const handleTestOllama = async () => {
    if (!formData.model) { toast.warning("Please enter an Ollama model name to test (e.g., llama3)."); return; }
    setIsTesting(true);
    try {
  ...
  const handleSaveConnection = async () => {
    ...
    setIsSaving(true);
    try {
      const meta = new storage.ConnectionMetadata({ id: formData.id, name: formData.name, provider: formData.provider, model: formData.model, createdAt: new Date().toISOString() });
    ...
    <div className="p-6">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <Card className="bg-[#1B2636] border-[#2A3B52] lg:col-span-1">
          ...
          <CardContent>{connections.length === 0 ? (<div className="text-center text-gray-400 py-8 px-4"><p className="font-semibold">No models connected.</p><p className="text-sm mt-1">Click 'New' to add your first connection.</p></div>) : (<div className="space-y-3">{connections.map((c) => (<div key={c.id} className={`p-3 rounded-lg cursor-pointer transition-colors ${selectedConnectionId === c.id ? "bg-[#4A90E2] bg-opacity-20 border border-[#4A3B52]" : "bg-[#0F1419] hover:bg-[#2A3B52]"}`} onClick={() => handleConnectionSelect(c)}><div className="flex items-center justify-between"><div className="flex items-center space-x-3"><CheckCircle className="w-4 h-4 text-green-500" /><div><p className="text-sm font-medium">{c.name}</p><p className="text-xs text-gray-400 uppercase">{c.provider}: {basename(c.model)}</p></div></div><Button size="icon" variant="ghost" className="h-7 w-7 hover:bg-red-500/20 text-gray-400 hover:text-red-400" onClick={(e) => { e.stopPropagation(); handleDeleteConnection(c.id, c.name)}}><Trash2 className="w-3.5 h-3.5" /></Button></div></div>))}</div>)}</CardContent>
        </Card>
