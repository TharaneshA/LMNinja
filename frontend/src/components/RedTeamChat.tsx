...
import { toast } from 'sonner'; // CORRECTED IMPORT: Import directly from the library
    ...
    <Card className="flex-1 flex flex-col bg-transparent border-none shadow-none mt-4 h-full">
          ...
          <div className="space-y-6">
            {messages.map((msg, index) => (
              <div key={index} className={`flex items-start gap-4 ${msg.role === 'user' ? 'justify-end' : ''}`}>
                ...
                <div className={`max-w-[75%] p-3 rounded-lg ${msg.role === 'user' ? 'bg-[#4A90E2] text-white' : 'bg-[#1B2636]'}`}>
            ...
            {isLoading && (
              <div className="flex items-start gap-4">
                <div className="w-8 h-8 rounded-full bg-primary flex items-center justify-center flex-shrink-0"><Bot className="w-5 h-5 text-primary-foreground" /></div>
                <div className="max-w-[75%] p-3 rounded-lg bg-[#1B2636] flex items-center">
      ...
      <CardFooter className="p-4 border-t border-[#2A3B52]">
        <div className="flex w-full items-center space-x-2">
          <Textarea
            placeholder={`Chat with ${activeModel?.name}... (Shift+Enter for newline)`}
            className="flex-1 bg-[#0F1419] border-[#2A3B52] resize-none"
            value={input}
            onChange={(e: ChangeEvent<HTMLTextAreaElement>) => setInput(e.target.value)}
            onKeyDown={(e: KeyboardEvent<HTMLTextAreaElement>) => { if (e.key === 'Enter' && !e.shiftKey) { e.preventDefault(); handleSendMessage(); }}}
          />
          <Button onClick={handleSendMessage} disabled={isLoading || !input.trim()} className="bg-[#4A90E2] hover:bg-[#3A7BC8]">
