...
import DetailedReport from './report/DetailedReport.vue';
...
<template>
  <div class="dashboard-container">
    <template v-if="activeView === 'report'">
      <DetailedReport :scan="selectedScan" @back="handleBackToDashboard" />
    </template>
      ...
      <n-spin :show="dashboardStore.loading">
        <div class="dashboard-content">
          <n-grid cols="4" :x-gap="16" :y-gap="16" style="margin-top: 16px;">
            <n-gi><StatCard label="Models Scanned" :value="dashboardStore.stats.modelsScanned" :icon="Server" /></n-gi>
            <n-gi><StatCard label="Total Scans Run" :value="dashboardStore.stats.totalScans" :icon="Zap" /></n-gi>
            <n-gi><StatCard label="Vulnerabilities Found" :value="dashboardStore.stats.vulnerabilitiesFound" :icon="Shield" color="#e88080" /></n-gi>
            <n-gi><StatCard label="Overall Pass Rate" :value="`${dashboardStore.stats.overallPassRate.toFixed(1)}%`" :icon="Checkbox" color="#63e2b7" /></n-gi>
          </n-grid>
          <n-card style="margin-top: 24px; flex-grow: 1; display: flex; flex-direction: column;">
            <n-tabs type="line" animated default-value="history">
              ...
              <n-tab-pane name="history" tab="Scan History">
                <ScanHistoryTable :scan-data="dashboardStore.scanHistory" @view-report="handleViewReport" />
              </n-tab-pane>
            </n-tabs>
          </n-card>
        </div>
