...
func (c *AnthropicClient) Query(ctx context.Context, prompt string) (string, error) {
	...
	requestBody, err := json.Marshal(anthropicRequest{Model: c.meta.Model, MaxTokens: 2048, Messages: []anthropicMessage{{Role: "user", Content: prompt}}})
	if err != nil {
	...
	if err := json.Unmarshal(bodyBytes, &apiResponse); err != nil {
		return "", fmt.Errorf("failed to unmarshal anthropic response (status %d): %s", resp.StatusCode, string(bodyBytes))
	}
	if resp.StatusCode != http.StatusOK {
		if apiResponse.Error.Message != "" {
			return "", fmt.Errorf("anthropic api error (%s): %s", apiResponse.Error.Type, apiResponse.Error.Message)
		}
		return "", fmt.Errorf("anthropic api returned non-200 status: %d", resp.StatusCode)
	}
